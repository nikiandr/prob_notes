...
\subsection{Математичне сподівання}\label{sec:expectation}
...
\begin{enumerate}
    ...
    \begin{proof}[Доведення для ДВВ]
        $\P\left\{\xi_1 = x_k\right\} = p_k$, $\P\left\{\xi_2 = y_j\right\} = p_j$, $\P\left\{\xi_1 = x_k, \xi_2 = y_j\right\} = p_{kj}$.
        В цих позначеннях
...
\end{definition}
\resume{enumerate}\label{eq:EXY_indep}
...
\begin{definition}\index{квантиль}
    ...
    % (що еквівалентно $F_\xi(x_0) \leq q$ та $\underset{x\to x_0 + 0}{\lim} F_\xi(x_0) \geq 1-q$) 
    для ДВВ.
\end{definition}
\begin{example}
...
\begin{enumerate}
    ...
    \end{proof}
    \item Генератриса --- аналітична функція в колі $|z| < 1$ (як наслідок властивості 1).
