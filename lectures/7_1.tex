...
\begin{tikzpicture}
\label{scheme}
\node[block] (a) {Збіжність з імовірністю 1};
\node[block, above right = 0cm and 2cm of a] (b) {Збіжність в середньому\\ квадратичному};
\node[block, below right = 0cm and 2cm of a]   (c) {Збіжність в середньому};
\node[block, below right = 2cm and -1.5cm of a]   (d) {Збіжність за ймовірністю};
\node[block, below = 1cm of d]   (e){Збіжність за розподілом};
\node[below left= 1cm and -3 cm of a]   (1){\circled 1};
\node[below right= 0.25cm and -1.9 cm of d]   (2){\circled 2};
...
Штрихова лінія \circled 1 ілюструє таку властивість: у кожній випадковій послідовності $(\xi_n,n\in\mathbb N)$, що збігається за ймовірністю, міститься підпослідовність $(\xi_{n_k},k\in\mathbb N)$, що збігається з імовірністю 1. Штрих-пунктирна лінія \circled 2 показує, що відповідний перехід справедливий, коли гранична випадкова величина $\xi$ є константою.
