...
Розглянемо випадковий вектор $\vec{\xi} = (\xi_1, \xi_2, ..., \xi_n)^T$, $\xi_k \sim {N}(a_k^o, \sigma_k)$ для $k=1,...,n$, координати незалежні у сукупності.
...
\begin{example}
    \begin{enumerate}
        ...
        \end{pmatrix}, \begin{pmatrix}
            1 & 2 \\
            2 & 6
        \end{pmatrix}\right) = {N}(\vec{a}, K)$. Знайти розподіл, характеристичну функцію, щільність розподілу 
        ...
        \end{pmatrix} + \begin{pmatrix}
            1 \\ 0 
        \end{pmatrix}$. Тоді $\vec{\eta} \sim {N}(\vec{d}, B)$,
...
Розглянемо випадок $n$-вимірного нормального розподілу $\vec{\xi} \sim {N}(\vec{a}, K)$ з виродженою матрицею $K$,
...
Усі значення $n$-вимірного гауссівського вектора $\vec{\xi} \sim {N}(\vec{a}, K)$ з виродженою кореляційною матрицею, що має ранг $m<n$,
зосереджені на $m$-вимірному підпросторі $\mathbb{R}^n$ --- $\mathrm{Im}K + \vec{a}$.
\subsection{Нормальний розподіл на площині} 
Розглянемо двовимірний гауссівський вектор $\vec{\xi} = (\xi_1, \xi_2)^T \sim {N}(\vec{a}, K)$.
...
\begin{example}
    $\vec{\xi} \sim {N}\left( \begin{pmatrix}
        -1 \\ 1
    \end{pmatrix}, \begin{pmatrix}
        1 & 2 \\
        2 & 16
...
Тоді ймовірність потрапляння в прямокутник $\Pi = \left\{(x;y)\in\mathbb{R}^2 : \alpha < x < \beta, \gamma < y < \delta\right\}$
