...
\begin{definition}
    \index{розподіл!гауссівський (нормальний)!багатовимірний}
...
\end{exercise}
Знайдемо сумісну щільність розподілу такого вектору:
\begin{gather*}
...
\begin{enumerate}
    ...
    \begin{proof}
        Було доведено, що з незалежності координат випливає їх некорельованість.
...
\begin{example}
    \begin{enumerate}
        ...
        $B^{-1} = \begin{pmatrix}
            67/242 & 19/242 \\
            19/242 & 9/242
        \end{pmatrix}$, $\det{B} = 242$.

        $f_{\vec{\eta}}(x_1, x_2) = \frac{1}{2\pi \cdot \sqrt{242}} \cdot \exp\left\{-\frac{1}{2}\left( B^{-1}(\vec{x} - \vec{d}), (\vec{x} - \vec{d}))\right)\right\} = $

        $ = \frac{1}{2\pi \cdot \sqrt{242}} \cdot \exp\left\{-\frac{1}{2}\left( \frac{67}{242}(x_1+1)^2 + \frac{38}{242}(x_1+1)(x_2-3) + \frac{9}{242}(x_2-3)^2\right)\right\}$.
...
у вигляді суми свого вектора математичного сподівання та вектора, перші $m$ координат якого є незалежними у сукупності та мають стандартний нормальний розподіл, 
...
\index{розподіл!гауссівський (нормальний)!на площині}
...
\begin{definition}
    ...
    $\vec{a} = \vec{0}, \; K = \begin{pmatrix}
        \sigma^2 & 0 \\
        0 & \sigma^2
    \end{pmatrix}$. У цьому випадку $f_{\vec{\xi}}(x, y) = \frac{1}{2\pi\sigma^2} \exp\left\{-\frac{x^2+y^2}{2\sigma^2}\right\}$, 
...
для якої $F_{\eta}(R) = \P\left\{\Vert \vec{\xi} \Vert < R\right\} = \begin{cases}
    ...
    0, & R \leq 0
\end{cases}$ --- це \emph{розподіл Релея} \index{розподіл!Релея}.
