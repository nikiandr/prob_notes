...
\noindent\textbf{Означення:}\index{розподіл!біноміальний}
...
Окремим випадком біноміального закону є \emph{розподіл Бернуллі}\index{розподіл!Бернуллі}: $\xi \sim \mathrm{Bin}(1, p)$.
...
\begin{center}
    \begin{tikzpicture}[
        ...
        every circle/.style={radius = 0.05}
    ]
     \pgfmathsetmacro{\s}{5};
     ...
     \foreach \i in {1,...,\n}:
        \draw[fill, thick] (\i-1, {binl(\i-1)*\s}) circle -- (\i, {binl(\i)*\s}) circle;
...
\noindent\textbf{Означення:}\index{розподіл!геометричний}
...
\end{exercise}
До геометричного закону можна звести \emph{закон розподілу Паскаля}\index{розподіл!Паскаля} $\xi \sim \mathrm{Pas}(a)$,
...
\noindent\textbf{Означення:}\index{розподіл!Пуассона}
...
$p_0(t+\Delta t) = p_0(t)\cdot\tilde{p}_0(t+\Delta t)$ для $m=0$. Застосуємо ординарність:
...
\noindent\textbf{Означення:}\index{розподіл!рівномірний}
...
\noindent\textbf{Означення:}\index{розподіл!експоненційний (показниковий)}
...
\begin{exercise}
    ...
    $\mathrm{Exp}(\lambda, x_0)$ зі щільністю $f_\xi(x) = \begin{cases}
...
\begin{enumerate}\index{потік Пуассона}
...
\noindent\textbf{Означення:}\index{розподіл!гамма}
...
в потоці Пуассона \index{потік Пуассона} з інтенсивністю $\lambda$. Таку версію гамма-розподілу іноді називають \emph{розподілом Ерланга} \index{розподіл!Ерланга}.
\begin{exercise}
...
\noindent\textbf{Означення:}\index{розподіл!гауссівський (нормальний)}
...
\begin{gather*}
    ...
    e^{-t^2} dt
\end{gather*}
Якщо $k = 2l + 1$, то $\beta_k = 0$, бо під інтегралом непарна функція.
...
\begin{enumerate}
    ...
    \item \emph{<<Правило $3\sigma$>>}: \index{правило $3\sigma$}
