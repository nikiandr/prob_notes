...
\begin{theorem*}\label{LLN}\index{теорема!Чебишова}
...
\begin{theorem*}[теорема Маркова]\index{теорема!Маркова}
...
\subsection{Посилений закон великих чисел}\index{закон великих чисел!посилений}
...
\begin{theorem*}[теорема Колмогорова]\label{SLLN}\index{теорема!Колмогорова}
...
\subsection{Закон великих чисел у схемі Бернуллі}\index{закон великих чисел!у схемі Бернуллі}
Розглянемо частинний випадок ЗВЧ Чебишова для схеми Бернуллі.
\begin{theorem*}[теорема Бернуллі]\index{теорема!Бернуллі}
...
\end{proof}
\begin{remark}
    Для практичного застосування цієї теореми важливою є оцінка збіжності $\frac{\xi_n}{n}$ до $p$:
...
\begin{theorem*}[теорема Пуассона]\index{теорема!Пуассона}
    Нехай $\xi_n$ задає кількість успіхів в перших $n$ випробуваннях з послідовності незалежних випробувань, в $k$-тому з яких ймовірність 
    успіху дорівнює $p_k$. Тоді
...
Історично одним з перших застосувань такого підходу було наближене обчислення числа $\pi$ за допомогою голки --- \hyperref[buffon]{задача Бюффона}. \index{задача!Бюффона} 
Проілюструємо застосування метода Монте-Карло для задачі наближеного інтегрування.

Нехай $f :[a,b]\to[0,+\infty)$ --- деяка неперервна невід'ємна функція, для якої потрібно наближено обчислити значення $\int\limits_a^b g(x) dx$. Запропонуємо два способи такого обчислення.

Спочатку розглянемо послідовність випадкових величин $\xi_n$, що мають спільний розподіл $\mathrm{U}[a; b]$.
$\E f(\xi_1) = \frac{1}{b-a} \int\limits_a^b f(x) dx$, тому $\int\limits_a^b f(x) dx = (b-a)\cdot \E f(\xi_1)$. 
З теореми Колмогорова (\ref{Kolm_theor}) відомо, що
$\frac{1}{n}\sum\limits_{k=1}^n f(\xi_k) \overset{\mathrm{P1}}{\longrightarrow} \E f(\xi_1), \; n\to\infty$, 
тому при достатньо великих $n$ виконується наближена рівність:
$$\int\limits_a^b f(x) dx\approx\frac{b-a}{n}\sum\limits_{k=1}^n f(\xi_k)$$
де $x_k$ --- реалізація величини $\xi_k$.

Другий спосіб розв'язання цієї задачі заснований на інтерпретації інтеграла як площі під графіком функції. 
Оскільки $f$ є неперервною функцією на відрізку, вона також є обмеженою: існує таке $M>0$, що $f(x)\leq M$ для будь-якого $x\in[a,b]$.
Тому, кидаючи випадкові точки в прямокутник $\Pi=[a;b]\times[0;M]$, ми будемо потрапляти у підграфік $G$ функції $f$ якраз з ймовірністю
$$p=\frac{\text{площа $G$}}{\text{площа $\Pi$}}=
\frac{\int\limits_a^b f(x) dx}{M(b-a)}$$
\begin{center}
    \begin{tikzpicture}
    [thick,dot/.style = {
        draw,
    ...
    \fill[lightgray] (1,0.01) -- plot[smooth] coordinates {(1,2) (2.5,4) (3.5,4) (5,1)} -- (5,0.01);
    \draw plot[smooth, thick] coordinates {(1,2) (2.5,4) (3.5,4) (5,1)}
    node[above left = 2.5 cm and 2.7 cm] {$f(x)$};
...
прямує до $p$.
Тому при достатньо великих $n$ виконується наближена рівність
$$\int\limits_a^b f(x) dx\approx M \cdot(b-a)\cdot \nu_n$$
