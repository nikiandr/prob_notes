...
\begin{enumerate}
    \item Якщо події $A$ та $B$ незалежні, то $\P(A\cap B) = \P(A)\cdot \P(B)$. Це випливає з (\ref{eq:cond_prob}) та (\ref{eq:indep_events}).
    \begin{remark}
...
Якщо $A_1, A_2, ..., A_n \in \mathcal{F}$ --- незалежні у сукупності, то, як було зазначено, з (\ref{eq:indep}) випливає, що
формула (\ref{eq:mult_for_n}) спрощується до $\P\left(\bigcap\limits_{k=1}^n A_k\right) = \prod\limits_{k=1}^n \P(A_k) = \prod\limits_{k=1}^n \left(1-\P(\overline{A_k})\right)$.
...
\end{proof}
У випадку зліченної кількості гіпотез формула (\ref{eq:total_prob}) доводиться аналогічно та перетворюється
