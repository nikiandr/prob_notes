...
\noindentОтже, $f_{\frac{\xi_2}{\xi_1}} = -\int\limits_{-\infty}^0 xf_{\vec{\xi}}(x, zx)dx + 
...
\begin{proof}
    ...
    \int\limits_0^{+\infty} \left({\iint_{\varphi^k(x,y) \geq z}} f_{\vec{x}}(x,y) dx dy\right) dz = \\
    =\iint\limits_{\mathbb{R}^2} \left( \int\limits_0^{\varphi^k(x,y)} dz\right) f_{\vec{x}}(x,y) dx dy = 
     \iint\limits_{\mathbb{R}^2} \varphi^k(x,y) f_{\vec{x}}(x,y) dx dy
    \end{gather*}
\end{proof}
...
\begin{example}\label{proof:expectation}
    ...
    $$E(\xi_1 + \xi_2) = \iint\limits_{\mathbb{R}^2} (x+y) f_{\vec{x}}(x,y) dx dy = 
    \iint\limits_{\mathbb{R}^2} x f_{\vec{x}}(x,y) dx dy + 
    \iint\limits_{\mathbb{R}^2} y f_{\vec{x}}(x,y) dx dy = E\xi_1 + E\xi_2$$
    Якщо $\xi_1, \xi_2$ незалежні, то $f_{\vec{x}}(x,y) = f_{\xi_1}(x) f_{\xi_2}(y)$.
    $$E\xi_1 \xi_2 =
