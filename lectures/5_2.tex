...
\begin{example}
    Нехай $\vec{\xi} = (\xi_1, \xi_2)$ рівномірно розподілені в квадраті $[0, 2]\times[0, 2]$.
    Знайти закон розподілу площі $S = \xi_1 \xi_2$.
    ...
    $F_S(z) = P\{\xi_1\xi_2 < z\} = \iint\limits_{\{xy < z\} \cap K} f_{\vec{\xi}}(x, y)dxdy$

    \begin{enumerate}
        \item при $z < 0$: $F_S(z) = 0$
        \item при $z \geq 4$: $F_S(z) = 1$
        \item при $0 < z < 4$: 
        
        \begin{tabular}{c p{8.8 cm}}
            \begin{tikzpicture}[baseline={(current bounding box.north)} ,scale = 1]
                \fill [lightgray, domain=0.5:2, smooth, variable = \x] plot ({\x}, 
                {
                    1/\x
                }) -- (2, 0) -- (0, 0) -- (0, 2) -- (0.5, 2);
                \draw [domain=0.333:3, smooth, variable = \x, ultra thick] plot ({\x}, 
                {
                    1/\x
                });
                \draw [black, ultra thick] 
            ...
            \end{tikzpicture} 
            &
            $F_S(z) = \frac{1}{4}S_{D_z \cap K} = \frac{1}{4} 
            (4 - \int\limits_{\frac{z}{2}}^2dx
            \int\limits_{\frac{z}{x}}^2dy) = 
            \frac{1}{4}(4 - \int\limits_{\frac{z}{2}}^2 
            (2 -\frac{z}{x})dx = $

            $= 1 - \frac{1}{4}\left.(2x - zlnx)\right|_{\frac{z}{2}}^2 =
            1 - \frac{1}{4}(4 - zln2 - z + zln\frac{z}{2}) = $

            $= \frac{1}{4}(z + zln2 - zlnz + zln2) = \frac{1}{4}(z + 2zln2 - zlnz)$
        \end{tabular}
    \end{enumerate}
    \begin{equation*}
        f_S(z) = 
...
\begin{example}
    $\xi_1$ та $\xi_2$ --- незалежні випадкові величини, $\xi_i \sim N(0, \sigma)$.
    
    \noindentЗакон розподілу $\eta = \frac{\xi_2}{\xi_1}$ --- ?

    За вже знайденою формулою та враховуючи незалежність маємо:
...
\begin{example}
    $\xi_1$ та $\xi_2$ --- незалежні випадкові величини, $\xi_i \sim Exp(\lambda_i)$, 
    $i = 1,2$.
    
    \noindentЗакон розподілу $\eta = \xi_1 + \xi_2$ --- ?
    ...
    \end{equation*}
    \begin{equation*}
        = \frac{\lambda_1\lambda_2}{\lambda_2 - \lambda_1}(e^{-z\lambda_1} - e^{-z\lambda_2})
        , z \geq 0 \text{ --- закон Ерланга 2-го порядку }(\lambda_1 \neq \lambda_2)
    \end{equation*}
\end{example}