...
Значення $F_{\vec{\xi}}(x, y)$ співпадає з імовірністю 
...
\hbox to \hsize{\hfil{
    \begin{tikzpicture}
        ...
        \path [pattern=north west lines, 
                pattern color=gray] (1, 1) rectangle (-0.55, -0.51);
        \draw [dashed] (-0.5, 1) -- (1, 1) -- (1, -0.5);
        \node [below] at (3, 0) {$x$};
        \node [above right] at (-0.5, 0) {$F_{\vec{\xi}}(x, y)$};
        \node [left] at (0, 3) {$y$};
    \end{tikzpicture}
}\hfil}
...
\begin{enumerate}
    ...
    \end{proof}
    \item $\lim_{x \to -\infty} F_{\vec{\xi}}(x, y) = 
           \lim_{y \to -\infty} F_{\vec{\xi}}(x, y) = 
           \lim_{x,y \to -\infty} F_{\vec{\xi}}(x, y) = 0$
    \begin{proof}
        Строге доведення грунтується на теоремах про неперервність 
        ймовірності.

        $\lim_{x \to -\infty} F_{\vec{\xi}}(x, y) = 
        \lim_{x \to -\infty} P\left\{\xi_1<x,\;\xi_2<y\right\} 
        = $

        $= \left|\text{використаємо теорему про неперервність 
        ймовірності}\right| = P(\varnothing \cap \{\xi_2 < y\})
        = P(\varnothing) = 0$.
