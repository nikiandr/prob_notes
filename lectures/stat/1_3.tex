...
\begin{definition}\index{довірчий інтервал}
...
$, де $\Phi(x)$ --- функція Лапласа. З \hyperref[table:laplace]{таблиці} її значень знайдемо відповідне значення $\varepsilon$ і отримаємо шуканий
...
значення $t_{\gamma}$ за допомогою відповідної \hyperref[table:student]{таблиці}. Шуканим значенням $\varepsilon$ буде $\varepsilon = \frac{t_{\gamma}}{\sqrt{n}} \cdot \sqrt{(\D^{**}\xi)_{\text{зн}}}$.
\begin{theorem*}[теорема Фішера]\label{Fisher_th}\index{теорема!Фішера}
...
$t_2$, що задовольняють $\P\left\{\frac{n \D^* \xi}{\sigma^2} > t_1 \right\} = \frac{1 + \gamma}{2}$ та
$\P\left\{\frac{n \D^* \xi}{\sigma^2}\geq t_2 \right\} = \frac{1 - \gamma}{2}$,
можна знайти за допомогою відповідної \hyperref[table:chi2]{таблиці}. Шуканим довірчим інтервалом буде
...
$\frac{n \D^* \xi}{\sigma^2}$ має розподіл $\chi^2_{n}$, тому $\frac{(n-1) \D^{**} \xi}{\sigma^2} \sim \chi^2_{n-1}$.
...
\index{довірчий інтервал!наближений}
...
\textbf{Кількість випробувань досить велика.} 
...
Користуючись \hyperref[table:laplace]{таблицею} значень функції Лапласа, для заданої довірчої ймовірності $\gamma$ знайдемо таке $t_{\gamma}$,
при якому 
...
\begin{tabular}{c p{10.4cm}}
    \begin{tikzpicture}[xscale=2.5, yscale=2.5, baseline={(current bounding box.north)}]
        \fill [lightgray] (0.47, 0.475) circle [x radius=0.7, y radius=0.2, rotate=40];
...
\textbf{Кількість випробувань мала.} 
